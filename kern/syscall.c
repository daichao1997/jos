...
sys_exofork(void)
{
	...
	e->env_tf.tf_regs.reg_eax = 0; // set the new env's return value to 0
	return e->env_id;
}
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	// LAB 4: Your code here.
	struct Env *e;
	struct PageInfo *pp, *oldpp;
	int r;

	if((r = envid2env(envid, &e, 1)) < 0)
		return r;

	if(((uint32_t)va >= UTOP) || (PGOFF(va) != 0))	// va >= UTOP, or va is not page-aligned.
		return -E_INVAL;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if((r = envid2env(srcenvid, &esrc, 1)) < 0)
		return r;	
	if((r = envid2env(dstenvid, &edst, 1)) < 0)
		return r;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if((r = envid2env(envid, &dstenv, false)) < 0)
		return r;
	...
	if((uint32_t)srcva < UTOP) {
		...
		if(perm & ~PTE_SYSCALL)	// only the bits in PTE_SYSCALL may be set
			return -E_INVAL;
		if(!(pp = page_lookup(curenv->env_pgdir, srcva, &entry)))	// srcva is not mapped
			return -E_INVAL;
		if(perm & ~(*entry) & PTE_W) // no write access to a read-only page
			return -E_INVAL;
	...
	// set return value for sys_ipc_recv, which actually never returns 0
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	switch (syscallno) {
		case SYS_cputs:
			...
			return a2; // return the length of output string
		case SYS_cgetc:
			return sys_cgetc();
		case SYS_getenvid:
			return sys_getenvid();
