...
static int init_stack(envid_t child, const char **argv, uintptr_t *init_esp, uint32_t dst);
...
*/
	...
	for (i = 0; i < elf->e_phnum; i++, ph++) {
		...
		// Load each segment to a seperated page, keeping its offset
		if ((r = map_segment(0, tmp_addr + PGOFF(ph->p_va), ph->p_memsz,
				     fd, ph->p_filesz, ph->p_offset, perm)) < 0)
			goto error;

		tmp_addr += ROUNDUP(PGOFF(ph->p_va) + ph->p_memsz, PGSIZE);
	}
	
	if ((r = init_stack(0, argv, &tmp_esp, tmp_addr)) < 0)
		return r;
	sys_exec(elf, tmp_esp);
	close(fd);
	fd = -1;
/*
	// Copy shared library state.
...
static int
init_stack(envid_t child, const char **argv, uintptr_t *init_esp, uint32_t dst)
{
	size_t string_size;
	int argc, i, r;
	char *string_store;
	...
	*init_esp = UTEMP2USTACK(&argv_store[-2]);
	...
	if ((r = sys_page_map(0, UTEMP, child, (void*)dst, PTE_P | PTE_U | PTE_W)) < 0)
		goto error;
...
copy_shared_pages(envid_t child)
{
	...
	for(uint32_t i = 0; i < UTOP; i += PGSIZE){
		if(!((pde = GET_PDE(i)) & PTE_P))
			continue;

		if(((pte = GET_PTE(i)) & (PTE_P | PTE_SHARE)) != (PTE_P | PTE_SHARE))
			continue;
		
		if((r = sys_page_map(0, (void *)i, child, (void *)i, pte & PTE_SYSCALL)) < 0)
			return r;
	}
	return 0;
}
